...
## [0.11.0] - 2026-03-27 [Author: Pavlo Kosov]
### Fixed
- Verified LCP optimization: `images.unoptimized` is set to `false` in `next.config.mjs`. No Next.js `<Image>` components are used in the critical render path — the game renders via `<canvas>`, which is not subject to LCP image optimization. (Fixes #46).
