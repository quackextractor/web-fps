...
export const LevelSelect: React.FC<LevelSelectProps> = ({
    ...
    return (
        <div 
            className="fixed md:absolute inset-0 flex flex-col items-center justify-center bg-black p-0 md:p-4 m-0 select-none pointer-events-auto"
            ...
            <div className="relative z-10 w-full md:max-w-5xl bg-black retro-border p-0 md:p-8 shadow-2xl overflow-y-auto max-h-full m-0" style={{ margin: 0 }}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 mb-4 md:mb-6 p-4 md:p-0">
                    {/* Mission list */}
                            ...
                            return (
                                ...
                                >
                                    <div className="flex justify-between items-start mb-2">
                                        <span className={`retro-text text-[9px] md:text-2xl font-bold ${isUnlocked ? (isSelected ? "text-red-400" : "text-red-600") : "text-gray-800"}`}>
                                            {(index + 1).toString().padStart(2, "0")}
                                        </span>
                                        {!isUnlocked && (
                                            <span className="retro-text text-[10.5px] md:text-[13px] bg-gray-900 p-1 border border-gray-800">LOCKED</span>
                                        )}
                                        {isUnlocked && (
                                            <span className={`retro-text text-[10.5px] md:text-[13px] ${getHazardColor(info.hazard)}`}>
                                                HAZARD {info.hazard}/5
                                            </span>
                                        )}
                                    </div>

                                    <p className={`retro-text text-[10.5px] md:text-[13px] font-bold truncate mb-1 ${isUnlocked ? "text-white" : "text-gray-700"}`}>
                                        {info.sectorName}
                                    </p>
                                    ...
                                    {isUnlocked && (
                                        <div className="flex gap-3 mt-2">
                                            {info.oreRedYield > 0 && (
                                                <span className="retro-text text-[10.5px] md:text-[13px] text-red-400">
                                                    RED ORE x{info.oreRedYield}
                                                </span>
                                            )}
                                            {info.oreGreenYield > 0 && (
                                                <span className="retro-text text-[10.5px] md:text-[13px] text-green-400">
                    ...
                    <div className={`retro-border bg-gray-950 p-4 flex flex-col justify-between min-h-[280px] ${selectedIndex === null ? 'hidden' : ''}`}>
                        {selectedInfo && (selectedIndex !== null) ? (
                            <>
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-y-2 gap-x-4 mb-4">
                                        <div>
                                            <p className="retro-text text-[6px] md:text-[8px] text-gray-500">HAZARD LEVEL</p>
                                            <p className={`retro-text text-xs md:text-sm ${getHazardColor(selectedInfo.hazard)}`}>
                                                {selectedInfo.hazardLabel}
                                            </p>
                                        </div>
                                        <div>
                                            <p className="retro-text text-[6px] md:text-[8px] text-gray-500">HOSTILES</p>
                                            <p className="retro-text text-xs md:text-sm text-white">{selectedInfo.enemyCount}</p>
                                        </div>
                                        <div>
                                            <p className="retro-text text-[6px] md:text-[8px] text-gray-500">RED ORE YIELD</p>
                                            <p className="retro-text text-xs md:text-sm text-red-400">{selectedInfo.oreRedYield > 0 ? `x${selectedInfo.oreRedYield}` : "NONE"}</p>
                                        </div>
                                        <div>
                                            <p className="retro-text text-[6px] md:text-[8px] text-gray-500">GREEN ORE YIELD</p>
                                            <p className="retro-text text-xs md:text-sm text-green-400">{selectedInfo.oreGreenYield > 0 ? `x${selectedInfo.oreGreenYield}` : "NONE"}</p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="flex flex-col items-center justify-center h-full">
                                <p className="retro-text text-[8px] md:text-[10px] text-gray-600 tracking-widest">SELECT A SECTOR</p>
                            </div>
                        )}
                    </div>
                </div>

                <div className="w-full md:w-1/3 mx-auto p-4 md:p-0">
