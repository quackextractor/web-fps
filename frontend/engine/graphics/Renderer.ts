import {
  ...
  WeaponType, AmmoType, PickupType, EnemyType
} from "@/lib/fps-engine";
...
export function drawWeapon(ctx: CanvasRenderingContext2D, player: Player, flash: number, screenWidth: number, screenHeight: number) => {
  const SCREEN_WIDTH = screenWidth;
  const SCREEN_HEIGHT = screenHeight;
